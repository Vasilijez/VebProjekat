...
public class PorudzbineController {
    ...
    @GetMapping("pregledKorpe") // menjao
    ...
    public ResponseEntity<String> dodajKomentar(@PathVariable UUID id, @RequestBody KomentarDto komentarDto, HttpSession session) throws Exception {
        ...
        if(!restoran.getNaziv().equals(komentarDto.getRestoran().getNaziv())) // zeznuto je onako preko objekata
        ...
        if(komentarDto.getOcena() == 0) // u javi je default vrednost za int 0, a ocene idu od 1-5 pa smo zasticeni
        ...
        komentar.setKupac((Kupac) ulogovani); // dodo
        komentar.setRestoran(restoran);// dodo
        komentarService.save(komentar);// dodo

        return new ResponseEntity("Uspesno napisan komentar", HttpStatus.OK);// dodo
    }
